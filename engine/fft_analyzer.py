...
if st.button("🔍 GENERATE DIAGNOSIS", type="primary", use_container_width=True, key="diagnose_btn"):
    with st.spinner("Menganalisis data dengan ISO 10816-3 + API 610 + IEC 60034-1..."):
        ...
        # === FFT FAULT DETECTION ===
        # Electrical Unbalance (2LF dominant + voltage imbalance)
        is_2lf_dominant = abs(peak3_freq - 100.0) < 5.0 and peak3_amp > 0.5 * peak1_amp
        if is_2lf_dominant and v_imbalance > 2.0 and phase_instability > 20:
            faults.append({
                ...
                "evidence": [
                    ...
                    f"Phase instability ±{phase_instability}° >20° (electrical origin)"
                ],
                "severity": "WARNING",
                "standard": "ISO 13373-2 Clause 5.4.3 + NEMA MG-1 §14.32"
            })
        
        # Mechanical Unbalance (1X dominant + phase stable)
        is_1x_dominant = abs(peak1_freq - fundamental) < 0.1 * fundamental and peak1_amp > 0.8 * (peak1_amp + peak2_amp + peak3_amp)
        if is_1x_dominant and v_imbalance < 2.0 and phase_instability < 10:
            faults.append({
                ...
                "evidence": [
                    ...
                    f"Phase stability ±{phase_instability}° <10° (mechanical origin)",
                    f"Fundamental = {fundamental:.2f} Hz (RPM/60)"
                ],
                "severity": "WARNING",
